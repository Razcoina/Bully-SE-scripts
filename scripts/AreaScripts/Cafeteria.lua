...
function F_CafCook(ped, path, node)
  ...
  if node == 2 and CookChance > 20 then
    LunchCook1 = true
  elseif node == 3 and CookChance > 40 then
    LunchCook2 = true
  end
end
function F_UpdateLUNCH()
  ...
  if LunchLadyModelLoad == true then
    while not RequestModel(58, true) do
      Wait(0)
    end
    shared.gEdnaID = PedCreatePoint(58, POINTLIST._AMBLUNCHLADY)
  ...
  if hour <= 19 and hour >= 7 and not MissionActive() then
    LunchLadyActive = true
  else
    LunchLadyActive = false
  end
  if not MissionActive() then
    if hour == 11 and minute > 30 or hour == 12 and minute < 59 then
      CafeLineActive = true
    else
...
function F_LunchLadyDestroy(triggerID, pedID)
  ...
  for i, ped in CafLineTable, nil, nil do
