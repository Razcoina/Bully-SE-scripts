...
function F_PropSet()
  ...
  for i, prop in tblCoverProps, nil, nil do
...
function F_DestroyDoors()
  while MissionActive() and not bDoorDead do
...
function F_GetOnSpudCannon()
  while MissionActive() and not bOnSpudCannon do
...
function F_Wave04()
  while MissionActive() and not bWave04Engaged do
...
function F_Wave03()
  while MissionActive() and not bWave03Engaged do
...
function F_Wave02()
  while MissionActive() and not bWave02Engaged do
...
function F_Wave01()
  while MissionActive() and not bWave01Engaged do
...
function F_GetKeyCode()
  SoundDisableSpeech_ActionTree()
  while MissionActive() and not bKeyCodeGiven do
...
function F_GetInfo()
  ...
  while MissionActive() and not bGotInfo do
...
function T_PlayerAsks()
  while MissionActive() and not bFattyInTheCan do
    if bNerdsHit then
...
function T_FattyScared()
  while MissionActive() and PedIsValid(gFatty) and not bPointOfNoReturn do
...
function F_Delete_Wave1_2()
  for n, nerd in tblNerd01, nil, nil do
...
function F_Wave4()
  ...
  CameraSetWidescreen(true)
  PlayerSetControl(0)
  for n, nerd in tblNerd02, nil, nil do
    if PedIsValid(nerd.id) then
      PedDelete(nerd.id)
    end
  end
  for n, nerd in tblNerd03, nil, nil do
    if PedIsValid(nerd.id) then
      PedDelete(nerd.id)
    end
  end
...
function T_Delete_Wave3_2()
  while MissionActive() and not bWave3Dead do
