...
function F_GetBack()
  ...
  while MissionActive() and not PlayerIsInTrigger(TRIGGER._4_S12_MSPHILLIPS) do
    if MissionTimerHasFinished() then
      bMissionSuccess = false
      return
    end
...
function F_CreateChasers(val)
  ...
  for i, guy in tblChasers, nil, nil do
    if guy.id == -1 then
    ...
    elseif i == 1 and not bChaser01FollowedPlayer or i == 2 and not bChaser02FollowedPlayer then
...
function F_ChaserStats(_ARG_0_)
  for _FORV_4_, _FORV_4_ in stat, nil, nil do
    PedOverrideStat(_ARG_0_, _FORV_4_.name, _FORV_4_.value)
  end
end
...
function F_PeanutCreate()
  ...
  PedPutOnBike(tblPeanut.id, _UPVALUE1_)
  PedSetAsleep(tblPeanut.id, true)
  VehicleSetOwner(_UPVALUE1_, tblPeanut.id)
...
function F_GiftInit(tbl)
  for i, tbl in tblItem, nil, nil do
    if i ~= 1 then
