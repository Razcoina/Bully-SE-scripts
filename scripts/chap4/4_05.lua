...
function F_CheckMascotStatus()
  if PedIsHit(pedMascot.id, 2, 100) or F_MascotIsTaunted() then
    ...
    if PedIsInTrigger(pedMascot.id, TRIGGER._4_05_MASCOTPOI1) and bPOI01Active then
      F_DispersePOI01()
    end
    if PedIsInTrigger(pedMascot.id, TRIGGER._4_05_MASCOTPOI2) and bPOI02Active then
      F_DispersePOI02()
    end
    if PedIsInTrigger(pedMascot.id, TRIGGER._4_05_MASCOTPOI3) and bPOI03Active then
      F_DispersePOI03()
    end
    if PedIsInTrigger(pedMascot.id, TRIGGER._4_05_MASCOTPOI4) and not bHitMascotInPOI4 then
      if F_PedExists(pedJockSprint01.id) then
        PedAttackPlayer(pedJockSprint01.id, 3)
      end
      bHitMascotInPOI4 = true
    end
    if PedIsInTrigger(pedMascot.id, TRIGGER._4_05_MASCOTPOI5) and not bHitMascotInPOI5 then
