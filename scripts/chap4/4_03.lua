...
local bDefend1, bDefend2, bDefend3 = false, nil, nil
local reserves = -1
...
function F_AnyJocksAlive()
  ...
  for i, entry in tblObjectsFound, nil, nil do
...
function F_DefendFirstBarricade()
  ...
  while not PlayerIsInTrigger(TRIGGER._4_03_TRIGGER_WAVE1) and not F_CurrentTargetBreached() do
    Wait(0)
  end
  ...
  for _, entry in tblNerdDefenders1, nil, nil do
...
function F_DefendSecondBarricade()
  ...
  local msgGetOnCannon, msgDefendObservatory = false, nil
  ...
  SoundPlayInteractiveStreamLocked("MS_EpicConfrontationLow.rsm", MUSIC_DEFAULT_VOLUME, 500, 500)
  while not MissionTimerHasFinished() and not PlayerIsInTrigger(TRIGGER._4_03_OPEN_DOOR) do
    Wait(0)
  end
  ...
  elseif MissionTimerHasFinished() then
    ...
    Wait(50000)
    while not MissionTimerHasFinished() and not PedIsPlaying(gPlayer, "/Global/WPCANNON/UseSpudCannon", true) do
      Wait(0)
    end
  end
  ...
  while not bDefend2 do
    ...
    if AuxiliaryTimer and GetTimer() - AuxiliaryTimer > 18000 then
      ...
      PAnimApplyDamage("scFieldObsGate", 0.881498, -111.463, 2.00091, 9000)
      PAnimApplyDamage("scFieldObsGate", 0.821367, -113.954, 2.055, 9000)
      F_CreateJockCover(17, POINTLIST._4_03_JOCK_B2_2W_L2, _UPVALUE9_, 1, POINTLIST._4_03_AUXCOVER2)
      F_CreateJockCover(16, POINTLIST._4_03_JOCK_B2_2W_P1, _UPVALUE9_, 1, POINTLIST._4_03_AUXCOVER3)
...
function F_PlaySpeechOnModel(model, event, dataTable)
  for i, entry in dataTable, nil, nil do
    --print("X")
...
function F_TableTakeCover(dataTable, bNeverLeave)
  ...
  for _, entry in dataTable, nil, nil do
    if entry.cover then
...
function F_JockAssaultManager(tableToCheck, reinforcements)
  for i, entry in tblJockWave1, nil, nil do
...
function F_NISFail()
  ...
  Wait(720)
  PedSetActionNode(gPlayer, "/Global/WPCANNON/Disengage/null", "Act/Props/WPCANNON.act")
  F_ClearPedTable(_UPVALUE0_, true)
  F_ClearPedTable(_UPVALUE1_, true)
